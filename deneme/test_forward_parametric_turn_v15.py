...
# Reuse only the stable Stage1/Stage2 entry helpers.
...
ns = {"__name__": "parametric_turn_v15_base", "__file__": str(AUTH_SOURCE)}
...
CAPTURE_HOLD_S = 0.75
VERIFY_S = 10.0
MAX_TEST_TIME_S = max(65.0, abs(TARGET_TURN_DEG) / 1.10 + 55.0)
...
def heading_hold_teacher(state, remaining):
    """Low-authority terminal hold; AFCS stays reduced and never fights the handoff."""
    yaw_rate = math.degrees(state["yaw_rate_rad_s"])
    roll = math.degrees(state["roll_rad"])

    desired_yaw_rate = float(np.clip(0.20 * remaining, -0.60, +0.60))
    rudder = float(np.clip(-0.80 * (desired_yaw_rate - yaw_rate), -0.55, +0.55))

    desired_roll = float(np.clip(0.18 * remaining, -2.0, +2.0))
    aileron = float(np.clip(0.25 * (desired_roll - roll), -0.50, +0.50))
    return aileron, rudder, desired_roll, desired_yaw_rate
...
print("=" * 120)
print("PARAMETRIC FORWARD-FLIGHT TURN V15 - TERMINAL HOLD")
...
try:
    ...
    # Keep Stage2 heading reference at the original heading. Lateral/yaw teacher owns the commanded turn.
    ...
    for step in range(int(MAX_TEST_TIME_S / dt)):
        ...
        if not captured:
            ...
            a2, a3, desired_roll, desired_yaw_rate = heading_hold_teacher(before, remaining)
        ...
        if not captured:
            ...
            if verify_s >= VERIFY_S:
                success = True
                termination = "turn_and_terminal_heading_hold_verified"
