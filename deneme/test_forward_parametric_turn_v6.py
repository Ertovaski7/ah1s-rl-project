...
RESULT_DIR = Path("results_forward_parametric_turn_v6")
...
ns = {"__name__": "parametric_turn_v6_base", "__file__": str(AUTH_SOURCE)}
...
# V6 altitude controller: intentionally asymmetric.
# V5 showed the key problem: below 300 ft the controller asked for too much
# upward recovery, then vertical momentum carried the helicopter through
# 300 ft and into a large climb. So upward recovery is deliberately slow,
# while overshoot above 300 ft is corrected more strongly.
COLLECTIVE_FF = 0.5830
ALT_TO_VS_GAIN = 0.10
MAX_UPWARD_VS_CMD = 0.25
MAX_DOWNWARD_VS_CMD = 1.50
VS_ERROR_TO_COLLECTIVE = 0.0060
PHYS_COLLECTIVE_MIN = 0.558
PHYS_COLLECTIVE_MAX = 0.592
COLLECTIVE_SLEW_PER_STEP = 0.0030
...
print("=" * 120)
print("PARAMETRIC FORWARD-FLIGHT TURN V6")
...
try:
    ...
    def turn_apply_action(action):
        ...
        desired_vs_raw = ALT_TO_VS_GAIN * (300.0 - alt)
    ...
    for step in range(int(MAX_TEST_TIME_S / dt)):
        ...
        if step % max(1, int(1.0 / dt)) == 0:
            print(
                ...
                f"COLL={last_alt['collective']:.4f} | V={state['forward_speed_fps']:6.2f}"
            )
