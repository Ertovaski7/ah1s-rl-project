...
RESULT_DIR = Path("results_forward_parametric_turn_v7")
...
ns = {"__name__": "parametric_turn_v7_base", "__file__": str(AUTH_SOURCE)}
...
# TURN CONTROLLER (unchanged from working V6)
...
# V7 VERTICAL CONTROLLER
# Main lesson from V6: altitude feedback alone is too late because the
# helicopter accumulates upward vertical momentum. Therefore vertical speed
# gets the dominant gain and positive climb is suppressed immediately.
COLLECTIVE_FF = 0.5790
ALT_TO_VS_GAIN = 0.060
MAX_UPWARD_VS_CMD = 0.15
MAX_DOWNWARD_VS_CMD = 1.50

# Asymmetric VS gains: climb must be killed aggressively; descent recovery
# stays gentler to avoid recreating the large overshoot from V5.
VS_GAIN_WHEN_CLIMBING = 0.0130
VS_GAIN_WHEN_DESCENDING = 0.0040

PHYS_COLLECTIVE_MIN = 0.535
...
print("=" * 120)
print("PARAMETRIC FORWARD-FLIGHT TURN V7")
...
try:
    ...
    last_alt = {"desired_vs": 0.0, "vs_error": 0.0, "gain": 0.0, "collective": COLLECTIVE_FF}
    ...
    def turn_apply_action(action):
        ...
        # If VS is positive, the helicopter is climbing: use strong damping.
        # If descending, use gentler recovery.
        gain = VS_GAIN_WHEN_CLIMBING if vs > 0.0 else VS_GAIN_WHEN_DESCENDING
    ...
    for step in range(int(MAX_TEST_TIME_S / dt)):
        ...
        if step % max(1, int(1.0 / dt)) == 0:
            print(
                ...
                f"VS={state['vertical_speed_fps']:+6.2f} | VS*={last_alt['desired_vs']:+5.2f} | "
                f"K={last_alt['gain']:.3f} | COLL={last_alt['collective']:.4f} | V={state['forward_speed_fps']:6.2f}"
            )
