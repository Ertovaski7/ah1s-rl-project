...
# ---------------- ALTITUDE: NEGATIVE TURN ----------------
# V16 -50 deg test showed correct yaw/roll direction but persistent descent:
# altitude 301 -> 279.9 ft while forward speed stayed healthy. The old branch
# used only 0.004 gain whenever VS was below desired VS, so it could not arrest
# descent quickly enough. Keep positive-turn tuning untouched and use a stronger
# descent-recovery branch only for TARGET_TURN_DEG < 0.
NEG_COLLECTIVE_FF = 0.5840
NEG_ALT_TO_VS_GAIN = 0.10
NEG_MAX_UPWARD_VS_CMD = 0.80
NEG_MAX_DOWNWARD_VS_CMD = 2.00
NEG_DESCENT_RECOVERY_GAIN = 0.010
NEG_CLIMB_DAMP_GAIN = 0.030
NEG_PHYS_COLLECTIVE_MIN = 0.470
NEG_PHYS_COLLECTIVE_MAX = 0.610
...
print("=" * 120)
print("PARAMETRIC FORWARD-FLIGHT TURN V17 - ASYMMETRIC ALTITUDE + PI HOLD")
print(f"REQUESTED TURN = {TARGET_TURN_DEG:+.2f} deg")
print(f"MAX TEST TIME = {MAX_TEST_TIME_S:.1f} s")
print("ALTITUDE MODE =", "NEGATIVE-TURN COMPENSATED" if TARGET_TURN_DEG < 0 else "V16 DEFAULT")
print("=" * 120)
    ...
    def controlled_apply_action(action):
        ...
        if TARGET_TURN_DEG < 0.0:
            desired_vs = float(np.clip(
                NEG_ALT_TO_VS_GAIN * (300.0 - alt),
                -NEG_MAX_DOWNWARD_VS_CMD,
                +NEG_MAX_UPWARD_VS_CMD,
            ))
            vs_error = desired_vs - vs
            # If aircraft is below desired VS, it is descending too fast or
            # climbing too slowly: use the stronger negative-turn recovery gain.
            gain = NEG_DESCENT_RECOVERY_GAIN if vs < desired_vs else NEG_CLIMB_DAMP_GAIN
            collective = float(np.clip(
                NEG_COLLECTIVE_FF + gain * vs_error,
                NEG_PHYS_COLLECTIVE_MIN,
                NEG_PHYS_COLLECTIVE_MAX,
            ))
        else:
            desired_vs = float(np.clip(
        ...
        last_desired_vs[0] = desired_vs
        last_alt_gain[0] = gain
        return controls
    ...
    for step in range(int(MAX_TEST_TIME_S / dt)):
        ...
        if step % max(1, int(1.0 / dt)) == 0:
            ...
            print(
                ...
                f"VS*={last_desired_vs[0]:+5.2f} | K={last_alt_gain[0]:.3f} | "
