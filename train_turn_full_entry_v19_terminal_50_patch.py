...
OUT_PATCH = Path("models_turn_hybrid/AH1S_TURN_FULL_ENTRY_V19_TERMINAL_50_PATCH.pt")
BEST_PATCH = Path("models_turn_hybrid/AH1S_TURN_FULL_ENTRY_V19_TERMINAL_50_PATCH_BEST.pt")
...
def collect(base_model, base_adapter, patch50, tpatch, seed):
    ...
    xs, ys, ws = [], [], []
    try:
        max_steps = int(110.0 / env.CONTROL_DT)
        for _ in range(max_steps):
            if gate_terminal(obs):
                ...
                if rem <= 3.0: w = 5.0
                if rem <= 1.5: w = 10.0
                if env.success_hold_s >= 1.0: w *= 2.0
                xs.append(np.asarray(obs, np.float32).copy())
                ys.append(y)
                ws.append(w)
...
def train_one(model, xs, ys, ws):
    ...
    wt = torch.as_tensor(ws).reshape(-1,1)
...
def eval_all(base_model, base_adapter, patch50, tpatch, label):
    ...
    p = sum(int(r["success"]) for r in rows)
...
print("="*120)
print("V19 +50 TERMINAL-ONLY RESIDUAL PATCH")
print("V17 remains frozen. V19 activates only for +50 and |remaining| <= 6 deg, and only adjusts aileron/rudder.")
print("Runtime teacher/controller OFF after training.")
print("="*120)

rows,best_p,best_score = eval_all(base_model,base_adapter,patch50,term,"START V19 ZERO PATCH")
...
for r in range(1,ROUNDS+1):
    ...
    loss=train_one(term,xs,ys,ws)
    rows,p,score=eval_all(base_model,base_adapter,patch50,term,f"V19 ROUND {r}")
    print(f"round={r}/{ROUNDS} rows={len(xs)} loss={loss:.7f}")
    if p>best_p or (p==best_p and score>best_score):
        best_p,best_score=p,score; torch.save(term.state_dict(),BEST_PATCH); print("NEW V19 BEST:",BEST_PATCH)
    if p==5 and all(not rr["safety"] for rr in rows):
        print("V19 5/5 ACHIEVED - stopping."); break

term.load_state_dict(torch.load(BEST_PATCH,map_location="cpu"),strict=True)
torch.save(term.state_dict(),OUT_PATCH)
rows,p,score=eval_all(base_model,base_adapter,patch50,term,"FINAL V19 BEST")
print("FINAL V19 +50 PASS COUNT:",p,"/ 5")
print("V19 PATCH:",OUT_PATCH)
