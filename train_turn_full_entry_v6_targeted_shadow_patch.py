...
BASE_ADAPTER_PATH = Path("models_turn_hybrid/AH1S_TURN_FULL_ENTRY_V4_RESIDUAL_ADAPTER.pt")
PATCH_PATH = Path("models_turn_hybrid/AH1S_TURN_FULL_ENTRY_V6_TARGETED_SHADOW_PATCH.pt")
BEST_PATCH_PATH = Path("models_turn_hybrid/AH1S_TURN_FULL_ENTRY_V6_TARGETED_SHADOW_PATCH_BEST.pt")

BASE_SCALE = torch.tensor([0.35, 0.20, 0.30, 0.30], dtype=torch.float32)
PATCH_SCALE = torch.tensor([0.18, 0.08, 0.24, 0.24], dtype=torch.float32)
...
def action_from_stack(base_model, base_adapter, patch, obs):
...
def evaluate_one(base_model, base_adapter, patch, target):
    ...
    try:
        ...
        for _ in range(max_steps):
            action = action_from_stack(base_model, base_adapter, patch, obs)
...
def evaluate_all(base_model, base_adapter, patch, label):
    rows = [evaluate_one(base_model, base_adapter, patch, t) for t in TARGETS]
...
def collect_target_shadow(base_model, base_adapter, patch):
...
def train_patch(patch, obs_np, tgt_np, wt_np):
    ...
    channel_w = torch.tensor([1.4, 0.5, 3.0, 3.0], dtype=torch.float32).reshape(1, 4)
...
if not BASE_ADAPTER_PATH.exists():
    raise FileNotFoundError(BASE_ADAPTER_PATH)

print("=" * 120)
print("FULL-ENTRY V6 - TARGETED +200 ACTOR-VISITED SHADOW PATCH")
print("Frozen PPO + frozen V4 adapter; only a small +200 patch is trained.")
print("Teacher labels actor-visited +200 states only; runtime teacher/controller OFF.")
print("=" * 120)

base_model = PPO.load(str(BASE_MODEL))
base_adapter = ResidualAdapter(BASE_SCALE)
base_adapter.load_state_dict(torch.load(BASE_ADAPTER_PATH, map_location="cpu"))
base_adapter.eval()
...
rows, best_passes, best_score = evaluate_all(base_model, base_adapter, patch, "START V6 ZERO PATCH")
torch.save(patch.state_dict(), BEST_PATCH_PATH)

for round_idx in range(1, ROUNDS + 1):
    obs_rows, tgt_rows, wt_rows = collect_target_shadow(base_model, base_adapter, patch)
    ...
    rows, passes, score = evaluate_all(base_model, base_adapter, patch, f"V6 SHADOW ROUND {round_idx}")
    if passes > best_passes or (passes == best_passes and score > best_score):
        best_passes, best_score = passes, score
        torch.save(patch.state_dict(), BEST_PATCH_PATH)
        print("NEW V6 BEST:", BEST_PATCH_PATH)
    if passes == 4:
...
rows, passes, score = evaluate_all(base_model, base_adapter, patch, "FINAL FULL-ENTRY V6 BEST")
print("\nFINAL FULL-ENTRY V6 PASS COUNT:", passes, "/ 4")
print("BASE PPO:", BASE_MODEL)
print("BASE V4 ADAPTER:", BASE_ADAPTER_PATH)
print("TARGETED SHADOW PATCH:", PATCH_PATH)
print("NOTE: base PPO and V4 adapter are frozen; runtime uses learned networks only, no teacher/controller.")
