...
if old_a not in text:
    raise RuntimeError("V17: post-turn handoff start not found")
...
if old_b not in text:
    raise RuntimeError("V17: previous_action reset not found")
...
if old_action not in text:
    raise RuntimeError("V17: Stage2 post-turn action block not found")
...
if old_print not in text:
    raise RuntimeError("V17: post-turn handoff print not found")
...
replacement = '''if abs(requested_turn) >= 300.0:
    ...
    # Rebase Stage2 smooth handoff from actual transition control state.
    handoff_collective = float(fdm["fcs/collective-cmd-norm"])
    handoff_elevator = float(fdm["fcs/elevator-cmd-norm"])
    handoff_action[0] = float(np.clip((handoff_collective - 0.620) / 0.030, -1.0, 1.0))
    handoff_action[1] = float(np.clip((handoff_elevator + 0.145) / 0.035, -1.0, 1.0))
...
if needle not in text:
    raise RuntimeError("V17: post-turn qualification loop not found")
text = text.replace(needle, replacement, 1)

print("=" * 120)
print("V17 DIRECT - 360 BALANCED TRANSITION TRIM -> SMOOTH STAGE2 HANDOFF")
print("Direct V11 base; no nested wrappers. Same FDM; runtime teacher OFF; thresholds unchanged.")
print("360 transition collective=0.575; max settle=20.0s.")
print("=" * 120)
